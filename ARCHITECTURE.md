...
2. **Agent Loop (`agent/core.py`):** The LLM (`llama-3.3-70b-versatile` via Groq) evaluates the prompt and the system instructions.
...
## Tool Components
* **Memory (`memory/manager.py`):** Utilizes a local ChromaDB instance to perform vector-based semantic searches of past interactions, allowing the agent to answer follow-up questions effectively.
* **Database (`tools/sql_tool.py`):** Translates natural language to SQL using the LLM. It validates the generated SQL against a strict, read-only Python guardrail before executing it against the `sqlite3` database.
* **Web (`tools/web_tools.py`):** Uses the `duckduckgo-search` library for keyless, free internet searching and `BeautifulSoup4` for HTML stripping and readable content extraction.